...
class New_Ad {
    ...
    public function __construct() {
        ...
        if (!array_key_exists("error", $result)) {
            $flash->success("Ad Saved successfully. Ad will be reviewed by admin for activation", null, TRUE);
            $images = $this->save_images($result['id']);
            header("Location: " . "/ad_site/post_ad.php");
        } else {
            $flash->error($result['error'], null, TRUE);
            header("Location: " . "/ad_site/post_ad.php", true, 307);
        }
    }
    ...
    private function new_ad() {
        ...
        try {
            $db = new Db ();
            $link = $db->connect();
            $query = "INSERT INTO ads (`userId`,category_id,title,description,price,`condition`,status,date_added) VALUES
                     ('" . $postData['user_id'] . "','" . $postData['child'] . "','" . $postData['title'] . "','" . $postData['description'] . "','" . $postData['price'] . "','" . $postData['condition'] . "','0',now())";
